...
def populate_fantrax_data():
    for week in season:
        if os.path.exists(fantrax_directory + 'fantrax ' + str(week) + '.csv'):
            ...
            CSV_to_Postgres.read_csv_file(fantrax_directory, 'fantrax ' + str(week), '.csv', False)
            time.sleep(0.05)
        else:
...
def populate_rotowire_data():
    for week in season:
        ...
        if os.path.exists(rotowire_directory + 'rotowire ' + str(week) + '.csv'):
            ...
            CSV_to_Postgres.read_csv_file(rotowire_directory, 'rotowire ' + str(week), '.csv', False)
            time.sleep(0.05)
        else:
